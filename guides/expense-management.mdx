...
1. Use `GET/bank_accounts` to get the **Bank Account ID** if the account already exists in the customer's accounting platform.
OR
...
1. Use `GET/accounts` to get the **Account ID** if the account already exists in the customer's accounting platform.
OR
2. Use `POST/accounts` to create an Account and get the **Account ID**.
...
Use the `POST/journal_entries` to reflect withdrawals and top-ups for a bank account/prepaid card.
...
1. To log a reimbursement request, use the `POST/bills` to create a bill containing the reimbursement amount.
    - Use the `POST/contacts` (filter on type= 'vendor') to create a specific vendor per employee you link to the Bill.
