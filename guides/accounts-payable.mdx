...
1. **Vendor**: This is the vendor linked to the Bill. Use the **GET/contacts** and **POST/contacts APIs**. A Contact ID is required to post a Bill, so if the contact doesn't exist on the accounting platform, create a new vendor using the **POST/contacts API**.
2. **Expense Account or Debited Account**: You can push multiple line items (usually just 1) when pushing an expense. Each line item will have an expense account associated (category = 'Expense'). The **GET/accounts API** lets you fetch all accounts, and you can filter the account type by **category: expense**.
