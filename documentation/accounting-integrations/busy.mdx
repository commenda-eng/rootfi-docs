...
The Busy on-premise connector offers users who use Busy a simple, safe and reliable way to share their accounting data through the Integration Labs platform. This connector is designed as a Windows application that can be installed on the user's local machine.

These steps are elaborated upon in detail below.

---

You can enable the Busy on-premise connector by enabling the integration from the 'Integrations' page on the Integration Labs Dashboard.
...
5. Copy and share the generated URL with your customer to integrate your user's Busy platform with Integration Labs.

The next steps outline the installation process for your user. The installation process for the Busy on-premise connector requires the user who is connecting their company data to complete the steps provided on the Integration Labs Connect Link/SDK. These steps must be followed carefully to ensure the successful installation and linking of the required company with the on-premise connector.
...
1. Secondly, the user needs to copy and paste the **Invite ID** from the Integration Labs Connect Link or Connect SDK and click on 'Next' to proceed to the next step.
