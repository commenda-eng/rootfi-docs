---
title: "RootFi Node.js SDK"
description: "Install the Node.js server-side SDK to interact with RootFi APIs both for Read and Write operations."
icon: "code"
---

## What is Node.js SDK?

A Node.js SDK is a software development kit that allows developers to create Node.js applications that use a specific API or service. SDKs typically provide pre-built code and libraries that make it easier to develop applications, and developers often use the Node.js SDK to build scalable and efficient server-side applications, including web servers, APIs (Application Programming Interfaces), and other networked software. It has gained popularity for its event-driven architecture and non-blocking I/O, making it well-suited for handling concurrent connections and building high-performance applications.

## Installation

RootFi's server-side helper libraries (also known as server-side SDKs) reduce the amount of work required to use RootFi's REST APIs, starting with reducing the boilerplate code you have to write. Below are the installation instructions for these libraries in a variety of popular server-side programming languages.

Open your project folder and run the following command on your command prompt to install the RootFi Node.js SDK or Install the library with [npm](https://www.npmjs.com/package/rootfi-api), a package manager for Node:

<CodeGroup>

```javascript Install the SDK
npm i rootfi-api
```

</CodeGroup>

Next, import the RootFi client in your application code:

```javascript
import { RootFiClient, RootFiError } from "rootfi-api";
```

## Instantiate RootFi

In your server file, instantiate the RootFi instance with your `apiKey`. You should generate the API keys on the [RootFi Dashboard](https://integrate.rootfi.dev/settings) and add them here.

```javascript
const rootfi = new RootFiClient({
  apiKey: "******************",
});
```

The resources can be accessed using the instance. All the methods invocations follow the namespaced signature:

## Run your first SDK request

Now that you have the Node.js SDK installed, you can access RootFi's APIs.

##### 1. [Core API](https://docs.rootfi.dev/api-reference/companies/get-all-companies)

##### 2. [Unified API](https://docs.rootfi.dev/api-reference/accounts/get-all-accounts)

To demonstrate Core APIs, we use the following JavaScript code snippet to make a Create request for creating an invite link. In this example, we create an invite link while specifying the `company name` and `integrations`:

```javascript
// Create an Invite Link
const data = await rootfi.core.inviteLinks.create({
  company_name: "Test Company",
  integrations: ["ZOHO_BOOKS"],
});
```

To demonstrate Core APIs for fetching a list of all companies, you can use the following JavaScript code snippet:

```javascript
// Get All Companies
const data = await rootfi.core.companies.list({});
```

To demonstrate Unified Accounting APIs, we are using the following JavaScript code snippet to make a List request for accounting data. In this example, we fetch all accounts for a specific connection based on certain filters:

```javascript
// Fetch all accounts for your connection
const data = await rootfi.accounting.accounts.list({
  filter: {
    rootfi_company_id: { eq: 1323 },
    rootfi_updated_at: { gt: "2023-05-12" },
  },
});
```
