...
Our Realtime APIs let you synchronously sync data from the Integration Platform in a unified format. For any data model we support, you can get data from the Integration platform in real time (synchronously).

## Fetch Unified Data on Demand

Realtime APIs are very powerful when you need to check certain data points on demand. It is especially useful for fetching an artefact like an invoice or small amounts of data that need to be displayed to the user, like Tax Rates.
...
While the most common use case would be to fetch a single piece of data, **our Realtime APIs are fully customisable and can call any underlying platform’s API** and unify the response for the data models we support.
