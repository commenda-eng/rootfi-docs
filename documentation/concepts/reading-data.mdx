...
8. **filter fields** - You can filter on any field in the data model. Each filter field parameter is a string that contains the field name, the operator and the value you want to filter on.
   ...
   2. **operator** - the operator you want to use to filter the data.
      Supported operators are `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `in`, `nin`, `is_null`, `not_null`, `contains`, `not_contains`
...
Our GET Apis in the [API Reference](/api-reference/v3/accounting/accounts/get-all-accounts) shows you how to use these parameters. Please contact us if you have any questions.
