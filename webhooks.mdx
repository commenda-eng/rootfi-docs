---
title: "Webhooks"
description: "The RootFi Platform offers robust webhook functionality, ensuring that your systems remain up-to-date with real-time data synchronization. With webhooks, as soon as data is synchronized from the accounting platform, the corresponding webhook is instantly activated. This guide will walk you through the steps to set up a webhook on the RootFi platform."
---
...
3. **Webhook Types & Configurations**

   - **Connection Changed Webhook**:
     This webhook is activated when a new connection is made via an invite link or if there's a change in connection status.

   - **Sync Completed Webhook**:
     This webhook is activated upon the completion of an asynchronous data sync. It can be initiated either manually for a specific company or when asynchronous data is written.

   - **Data Model Changes Webhook**:
     - You must specify the Event Types for this webhook:
       ...
       - **DELETE**: Activated if data is removed from the Accounting platform, but only after a full_sync operation.
     - Select the specific data models to activate this webhook. Options can be chosen from a dropdown list or you can select all.
